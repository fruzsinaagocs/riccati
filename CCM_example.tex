...
\begin{noframe}
    ...
    \node[squarednode, align=center]      (cheb)  [left =1.0 cm of conv] {Chebyshev step \\ in $u(t)$}; 
    \node[squarednode, align=center]      (ic)  [below=0.4cm of conv] {Compute $x$ from $u$, \\ match $u(t_i), u'(t_i)$};     
    \node[squarednode]      (accept)  [below =3.8cm of switch] {};    
    ...
    \node[inner sep=0, minimum size=0, above=7.87cm of m] (n) {}; % invisible node
    \node[inner sep=0, minimum size=0, above=2.6cm of switch] (o) {}; % invisible node    
    ...
    \draw[->] (k) -- (cheb.north) ;
    \draw[->] (conv.south) -- (ic.north) node[midway, below=-0.5ex] {\tiny True};    
