...
def test_solve():
    ...
    yi = sp.airy(-xi)[0]
    dyi = -sp.airy(-xi)[1]
    xs, ys, dys, ss = riccati.solve(w, g, xi, xf, yi, dyi, eps = eps)
    xs = np.array(xs)
    ys = np.array(ys)
    ss = np.array(ss)
    ...
    ax[1].loglog(xs[ss==True], yerr[ss==True], '.', color='C0', label='Success')
    ax[1].loglog(xs[ss==False], yerr[ss==False], '.', color='C1', label='Failure')
    ax[0].set_ylabel('Solution, $y(x)$')
    ax[1].set_ylabel('Relative error, $|\Delta y/y|$')
    ax[1].set_xlabel('$x$') 
    ax[0].set_title('Numerical solution of the Airy equation')
    ax[1].legend()
    plt.tight_layout()
    plt.show()
