...
when the solution has highly oscillatory and smooth regimes. % AB
...
WKB has recently been exploited in numerical methods. These algorithms achieve
$\bigO(1)$ (frequency-independent) runtime in regions where the asymptotic
series is a sufficiently good approximation.
% note the pivot, since it's a surprise
...
x_0 = \pm i\om, \\
    x_j' - x_{j-1}^2 + 2x_j x_{j-1} + \om^2 = 0, \quad \text{for } j = 1, 2, \ldots, N, 
\end{gather}
...
\Fruzsi{I don't think the reader needs to know the details of the Cheby discretization until later, this might be adding too much detail too soon, but I'm happy to keep this paragraph here.}
...
\begin{figure*}[tb]
    \centering
    \subfloat{\includegraphics[width=0.5\textwidth]{plots/complexsetup.pdf}}
    %\label{residual-reduction}
    \subfloat{\includegraphics{plots/convergence.pdf}}
    \caption{\label{complex-setup} Bla }
\end{figure*}
%
...
\ee
where Ai, Bi are the Airy functions of type ...\Fruzsi{cite DLMF/AbramoviczStegun}
...
%
\Fruzsi{TODO: give this method a name: RDC (Riccati defect correction), and fix plots).}
\begin{description}
\item[The %\AB{Kummer?}
