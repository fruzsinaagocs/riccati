...
%There are some commented lines just before the bibliography (starting '% Some things left to do') which I think would be worth doing, but I'd like your opinion on this.

%There is an entire subsection dedicated to the cosmology example you've seen before, commented out. I didn't have a good place for this and the paper is already over 20 pages long.
...
\AB{overall: is there a way to make \cref not write Eq.~(3) but instead
  write (3)? This is standard; Eq. everywhere is arcane.}
...
\AB{I don't get this last ``ways'' - are you thinking of nonlin vs lin, or systems vs scalar, or nth-order vs 2nd-order? We know you need at least 2nd ord or 2 coupled 1st ord to get osc, but then there is only one mechanism. Suggest cut.}

In this work we focus on ODEs of a \AB{particular $\to$ commonly occurring}
form: the general \AB{$\to$ variable-coefficient} linear, second
...
\AB{need to back this up: can say that in early-universe sims need
  to run millions of such ODE solns at different parameters, so solution needs to be very rapid. People tend to assume ODEs are solved, so some modern motivation needed.}
...
Ah, this one has some refs in the intro, maybe add a couple of the
most compelling ones:
https://arxiv.org/pdf/2205.01781.pdf
}
\AB{Maybe something on (or in concl):
  our proposed method could give a simple and efficient numerical
  replacement for complicated asymptotic analytic methods
  https://www.math.leidenuniv.nl/~vivi/stud_sem2011/BosleyKevorkian92.pdf
  https://arxiv.org/pdf/2205.01781.pdf
  }
...
\AB{The following is IMHO too much detail about a method we
  don't use. Could we cut it down, or if we must present the Riccati
  form, do it on our ODE using $\om$ large and no $\varepsilon$.
  Then we get to reuse this later.}
A
% large  % it's not a large class I would say
class of efficient numerical solvers for highly oscillatory ODEs of the
...
\AB{is there a way we can rewrite this mini tutorial on WKB using $\om$ large,
  not $\varepsilon$ small? It is confusingly different from our later asymptotics.}
...
\ee
\AB{Note this is a great opportunity to set up the notation for our method here, ie to make this {ricc}. Then set $g\equiv 0$ for the WKB only tutorial.}
...
\begin{align}\label{wkbrecur}
    x_0 &= \pm i\om,\\
    x_i &= -\frac{1}{2x_0}\left( \dot{x}_{i-1} + \sum_{k = 1}^{i-1}x_k x_{i-k} \right). \mbox{\AB{is this for $i=1,2\dots$ ?  And could combine on one line to make this WKB section more compact}}
\end{align}
\AB{Notation: better to use $j$ not $i$ here, since that's the symbol
  we use for iteration later. Always keep a symbol consistent when possible to reduce confusion}
Although originally an analytic tool,
WKB has recently been exploited in numerical methods.  % note the pivot, since it's a surprise
...
expansion truncated after $N = 2$
\AB{what is $N$? The reader does not know}
to transform \cref{odewkb} into a smoother
problem.
\AB{good}

\AB{Now is good place to state the disadvantages of the above to motivate our work:
  WKB order is fixed, giving $\bigO(h)$ convergence in the numerical
  step size $h$, with a small prefactor.
  So to get high accuracy need very small $h$ - 
  Can cite your own graph for this in oscode paper.
  While useful for 3 digits of accuracy, it is inefficient for more than 6.
  Or something - you have alreday phrased this well in your talk.
  }
...
\AB{again, $i=$ whatever. Or $j$}
\end{gather}
and solving them analytically. In \cref{phasefun}
...
\AB{What happened to Krivec 2006, 2014 QLM?
And Mandelzweig 2004 is the clearest presentation.
  We should discuss them.}
\AB{Now is the time to explain why QLM is not great:
  the solution needed at each step itself an oscillatory 1st order ODE.
  Explain why this is then problematic.
  Also, is the QLM series computed *numerically* like we do, or is it
  a mess of algebra? (semi-analytic?)
  }
...
Riccati phase function,
which has several advantages
\AB{no windowing, damping simply included}.
meaning simply that the
phase function is the logarithmic derivative $x(t) = (\log u(t))'$ from \cref{riccwkb}.
\AB{good that you tied it into prior equation. It will be perfect
  when we fix that prior eqn to be \{ricc\}.}
%
...
\AB{Yes! This is a great point, and should come before the above
  two more detail-oriented comments.}
...
%
\AB{all great points, and move them up to before the comparison to trad WKB}

\AB{This para I did some direct edits, since close:}
The phase function method, as with WKB or QLM methods,
is naturally only applicable in the high frequency regime.
%(but not WKB-marching \AB{I don't understand how WKB-marching works in the
%  non-osc regime?}
%or \texttt{oscode})
%is that it is
Thus, in the present work, we borrow ideas from \cite{oscode} to
...
%like the latter two, can deal with both regions where the
%solution is oscillatory (and the asymptotic expansion used to represent it is
%sufficiently accurate) and when it is not.
This is achieved by switching
between the Riccati
asymptotic expansion and a conventional
...
$\bigO(h^6)$ error.
\AB{the following is a cruical point that should move above to just after
  where oscode
  introduced, to motivate our work:}
...
\AB{Valiant, but I think too much detail here. Just say we are spectral.}
%
...
% Some things left to do:  (are this FA or AB ones? Are some obsolete?)
% AB comments with "-"
