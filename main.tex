...
uncontrolled approximate asymptotic methods.
...
\begin{equation}\label{wkbrecur}
  x_0 = \pm i\Omega, \qquad x_j = -\frac{1}{2x_0}\left( \dot{x}_{j-1} + \sum_{k = 1}^{j-1}x_k x_{j-k} \right), \quad \text{for } j = 1, 2, \ldots.
...
%$\bigO(h^6)$ error.
%
In terms of the time-step $h$, neither method is high-order.
\texttt{oscode}'s has
% AB: local?
error $\bigO(h)$, although
its prefactor can be small when $\om_0$ is large.
...
%of these two methods having low $h$-order and fixed asymptotic
%order is
a small requested tolerance in either method forces $h<1/\om_0$.
...
(QLM) \cite{bellman1970} instead approximates the solution
of \cref{riccwkb} with a series of iterates $x_i(t)$ by converting it to the
...
\label{qlmode}
\ee
and either solving them symbolically \cite{mandelzweig2004,krivec2006}
or numerically \cite{krivec2008,krivec2014}. The symbolic approach
