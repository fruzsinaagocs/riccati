...
If dense output is required, \ie the solver is
to perform interpolation and compute the numerical solution between
internal time nodes, then this is done by constructing the integration matrix
$Q_n$ mapping function values at $n$ Chebyshev nodes to values of the
integral of the interpolating polynomial at those points (with the last value
being zero). We use \texttt{chebfun}'s \cite{chebfunguide,chebfunquad}
implementation which constructs $Q_n$ as $Q_n = T_n B_n T_n^{-1}$, where $T_n$
and $T_n^{-1}$ convert Chebyshev interpolation polynomial coefficients to
node values and vice versa, and $B_n$ is the appropriate Vandermonde matrix. $T_n$
($T_{n-1}$) is equivalent to the (inverse) discrete cosine transform
\cite{gentlemanclenshawI,gentlemanclenshawII}. The matrix $Q_n$ is computed once and for all at
$\bigO(n\log n)$ cost, then at each timestep $Q_n \mbf{x}$ is performed,
followed by a rescaling by $h/2$.
If the solution is only required at the end of the interval, $t = t_1$, then
$z$ need not be evaluated at the Chebyshev nodes and it suffices to compute the
definite integral $\int_{x_i}^{x_i+h}x\mathrm{d}t$ at each timestep. This is
done by Clenshaw-Curtis quadrature, following \cite[Chapter 12]{tref}.
...
with an example.
In the left panel of \cref{residual-plot}, we apply
...
with a rate well described by $\om_{\text{max}}^{-j}$.
...
convergence is extremely rapid, with machine precision reached at $j=5$.
% AB: new thought...
Interestingly, at moderately large $\om_{\text{max}} \approx 10^3$ the initial decay is
still very useful, but the rate decreases after $j=6$; we
suspect this ``kink'' is due to
the discrete nature of the $D_n$ matrix diverging from
the true derivative after repeated applications. Since the residual for
constant $\om$ is zero analytically, we model the
discretization error by taking $\om(t)$ to be constant and monitoring the
residual while iterating. For fixed $n$ and $h$, smaller values of constant
$\om$ result in larger discretization errors after $j > 1$ iterations,
therefore we fix the value of the constant to be the minimum of $\om(t)$ in
\cref{bursteq} over the interval $t \in [0, 0.5]$.  
The resulting error model is shown as dotted lines in
the right panel of \cref{residual-plot}. We observe that in this example
the discretization error becomes dominant before the theoretically predicted
minimum residual occurs at the optimal stopping point (after $\bigO(\om)$
iterations), which explains the kink phenomenon. The residual's progression
after the kink is in agreement with the discretization error.
%(and the convergence rate of spectral differentiation)
The exact position of the kink is determined by a number of parameters,
including: the ratio of the distance to the nearest pole of $\om$ and the
stepsize $h$; the number of nodes $n$; and the magnitude of $\om$. Detailed
investigations into how each of these parameters affect the discretization
error are deferred to future work.
%growth in the
...
\begin{figure*}[tb]  
    ...
    \subfloat{\includegraphics{plots/residual-k-errormodel.pdf}}
    \caption{\label{residual-plot}
      Demonstrations of Riccati residual defect correction.
      ...
      The right plot contains the same information as the left and an additional
      set of dotted lines corresponding to our model of the roundoff error in
      the discretized Riccati iteration, computed by setting $\om$ to be
      constant and equal to $\min_{t\in [0, 0.5]}\om(t)$ with $\om(t)$ defined in
      \cref{bursteq}. As the discretization error becomes dominant, the
      residual's geometric decay slows or turns around.
      %
      %     
...
of $n$.
If the relative difference falls below the user-specified
threshold $\varepsilon$, the step with doubled $n$ is accepted ($t_i$ is advanced by $h$).
...
\be
  ...
  % AB: not sure that deserves a separate formula, esp since want look simpler
    ...
    \ee
    % AB got here
where $\veps_h$ is a parameter quantifying relative tolerance (separate from $\veps$). In the above, the $1/(n+1)$ in the exponent is justified by noting that the
...
16-core, 3.6 GHz workstation with 264 GB of RAM. No attempt has been made to
parallelize our code, and for the sake of fair comparison the experiments were
run with the \texttt{OMP\_NUM\_THREADS=1} flag.
...
experiments in \texttt{Python} is available on GitHub\footnote{\url{https://github.com/fruzsinaagocs/riccati}}.
...
\texttt{numpy} 1.23.1, \texttt{scipy} 1.9.3.
% FA: Python, numpy versions verified  once experiments ran again. 
...
Our solver covers around $10^{11}$ periods using $\approx 30$ timesteps,
taking $\approx 10$ milliseconds of CPU time,
% FA: It's only 30 timesteps, not 10^3 :)
...
\cref{convergence-airy} demonstrates the convergence
...
down to the theoretical minimum error \cref{mineps}.

\begin{figure}[tb]
    \centering
    \includegraphics{plots/airy-numsol-newK.pdf}
    ...
    best achievable accuracy \cref{mineps}
    in blue. On the
...
\begin{figure}[tb]
    \centering
    \includegraphics{plots/convergence-newK.pdf}
    \caption{\label{convergence-airy} Error performance of the solver when
    applied to the Airy equation, \cref{airy}--\cref{airyic}. The plot shows
    the achieved relative error of the solver against the supplied tolerance
    for the example in \cref{airy-demo}, while the upper end of the integration
    interval is varied. The dotted lines show the maximum possible accuracy for
    each interval as per \cref{conditionnodef}, which the solver indeed
    achieves. %to within about one digit.
    % FA: Oops, with the factor of 2\pi we actually do better!
}
\end{figure}
...
\item[Kummer's phase function method:]{ an arbitrarily high-order solver
        ...
        transitions to nonoscillatory behavior. The user provides $\om$ as a callable.
...
%        This recently developed  ... well, all are :)
        % FA: You told me to emphasize this in a previous comment.
...
\cref{bremer237tab} presents the runtime statistics of ARDC, Kummer's phase
function method, the WKB marching method, and \texttt{oscode} respectively. 
%For \texttt{oscode}, the table contains two separate entries that have
...
%limit. 
For all solvers, we set the relative tolerance to $\veps=10^{-12}$, and
...
$n = 16$. For all other methods, none of the input parameters were modified from their default values
...
\begin{description}
    ...
    \item[$\bm{t_{\mathrm{solve}}}$:]{Total runtime, in seconds, of a single
      ...
      % FA: Removed -- the user can decide whether it's important or not.
...
\begin{table*}[tb]
    ...
    \resizebox{\textwidth}{!}{\input{tables/new/all/bremer237-all-1e-12}}
    \caption{\label{bremer237tab} Accuracy, runtime and evaluation statistics of the algorithms
      considered  when applied to \cref{bremer237eq}.
      See \cref{solvercomp} for a summary of the solvers being compared and
      their settings, and an explanation of column headers.}
\end{table*}

\cref{bremer237-timing} provides a visual comparison of the runtime (left) and
achieved relative error (right) as a function of the frequency parameter
$\lambda$ of the solvers being tested. In addition to runtimes and relative
errors at $\veps = 10^{-12}$ reported in \cref{bremer237tab} (solid lines), it
also shows those at $\veps = 10^{-6}$ (dashed lines). It further shows the same
quantities for a 7,8th order Runge--Kutta method (denoted RK78) \cite{HWN,dop853} as a
representative of many standard high-order methods, whose runtime is expected
to grow as $\bigO(\lambda)$. The right-hand panel contains a grey shaded area
whose upper edge
serves as an upper bound on any data points (relative errors) that fall within it. This area is
defined by the error reported for Kummer's phase function method in
\cite[Table~1]{bremer2018}, which being our reference for computing errors
gives an upper bound. This is reflected in \cref{bremer237tab}: for any
measured errors lower than the reference we report only the upper bound.
...
function evaluations and steps become constant ($\approx 2$), causing the runtime to be
constant as well. Towards $\lambda = 10^7$, only $\approx 2$ terms are required for the
asymptotic expansion to achieve an estimated (local) relative error of
$10^{-12}$. At $\veps = 10^{-12}$, ARDC achieves the same accuracy as the
Kummer's phase function method, in the intermediate-to-large $\lambda$ regime, but is roughly 10 times faster.
 \cref{bremer237-timing} illustrates this nicely, but highlights
that Kummer's phase function method becomes significantly faster and comparable 
to ARDC at $\veps = 10^{-6}$, without the loss of accuracy, at all frequencies.
In other words, the convergence curve (achieved versus demanded accuracy) in
Kummer's phase function method's implementation is closer to a step function
...
happens later, at around $\lambda \geq 10^4$. This manifests itself as the
low-$\lambda$ ``hill'' in the left panel of \cref{bremer237-timing} at $\veps = 10^{-12}$,
but disappears at the higher tolerance $\veps = 10^{-6}$. 
...
constant as the frequency increases. As with Kummer's phase function method, at
...
\texttt{oscode} potentially overestimating its local error.

Finally, the WKB marching method needs significantly fewer function evaluations
(although achieves fewer digits of accuracy) at high frequencies because it
asks the user to provide high-order derivatives of the frequency $\om(t)$,
therefore it need not use further evaluations of $\om$ to compute said
...
% FA: Does this comment still stand? Hopefully it reads more like a discussion of results now.
\begin{rmk}
Three solvers (ARDC, the
Kummer's phase function method, and \texttt{oscode}) offer a dense output
option, and do so by interpolating a slowly-varying phase function,
...
\begin{figure}[tb]
    ...
    only accurate up to the upper edge of the grey region. This is consistent with the theoretical error minimum \cref{mineps}.}
\end{figure}
...
at the internal steps of ARDC as test points $t \in [0.0, 0.9]$.
% AB some comment about why [0,0.1] and [0.9,1] are problematic.
% FA: Update, [0.0, 0.1] no longer problematic -- was programming issue, not fundamental.
Since both $\om(t) = \sqrt{\nu(\nu+1)/(1-t^2)}$ and $\gamma(t) = -t/(1-t^2)$ have singularities at $t = \pm 1$, 
we avoid the proximity of $t = 1$.  
% FA: Should we say anything else? For \nu = 10^9, I tested the solver out to t = 0.999, it works fine.
Initial conditions at $t_0 = 0$ are chosen to match the value and derivative of
$P_\nu$, expressible in term of the gamma function, see
\cite[Chapter~14.5]{dlmf}. In practice, we used \texttt{scipy}'s special
function library to set initial conditions and to evaluate the numerical error,
which at large $\lambda$ became the computational bottleneck in our experiment,
taking $\bigO(1)$ \si{s} per target time. If ARDC were to be used for special
function evaluation, two (slow) evaluations of $P_\nu$ by standard means are
unavoidable to fully specify the initial conditions, but further evaluations,
as we will demonstrate, are computationally cheap.

% Initial conditions at $t_0=0.1$ are chosen to match the value
%and derivative of $P_\nu$.
% ? AB I'm guessing. please correct this, but it has to be explained.
%
...
%linearly combined to satisfy initial or boundary conditions.
% AB: that is a true general statement, but why is it relevant here: are you trying to propose a method to evaluate special funcs without ICs? It's unclear what the goal is.
In this example,
...
% AB: This is much less than n=40 from before - why?
% Imprecise again, number of nodes is (n+1) - AB, fine
...
% AB: well, we don't list kappa for the prob - would be needed to make that
% claim.
For any of the $\nu$ values tested, evaluation at new target points $t$ may
then be performed in around $10^{-6}$ \si{s} per target point.
% PLEASE INSERT A TIMING QUOTE HERE.
% FA: Done -- we have dense output now!

%Note that after the solution of the ODE has taken place, evaluating $u$ at intermediate points (\ie not at the internal timesteps taken by the solver) is possible via interpolation of the phase function $x$ in $\bigO(1)$ time, independently of the degree $n$ in \cref{legendreode}.


\begin{table}
    \centering
    \renewcommand{\arraystretch}{1.2}
    \input{tables/new/rdc/legendre-rdc.tex}
    \caption{Accuracy, runtime and evaluation statistics of the proposed ARDC method when
      ...
      % AB they are not identical : osc, slo -> Ricc, Cheb. Which pair is it going to be?
      % FA: I made them identical.
...
%first decsribe method, how improves in acc
%over WKB-type, and in generality/simplicity over Kummer phase type.

The proposed Riccati defect correction is not convergent, merely asymptotic
with respect to large $\om$.
However, we proved (\cref{TR}) that, for analytic coefficients with
...
We leave it to future work to rigorously analyse the \emph{discretized iteration}
(\cref{discr}), and its associated kink phenomenon visible in \cref{residual-plot}.
...
post-processing step by solving the ODE with two sets of linearly independent
...
given accuracy. This application will be explored in a future paper. A fast
oscillatory solver paired with efficient oscillatory quadrature may then be used to
solve inhomogeneous linear ODEs with slowly varying forcing terms, as recently
pointed out by \cite{serkh2022}.
% PDEs???
...
% FA: Already mentioned earlier in conclusions.

% inhomog, cite Serkh--Bremer arxiv 2022.
% FA: comment and citation inserted.
