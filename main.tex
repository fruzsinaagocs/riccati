...
\emph{local frequency} function.
For now, we only consider the case when $\om$ is purely real, 
...
x = \sum_{j=0}^{N} x_j\varepsilon^i,
\ee
and match powers of $\varepsilon$ in \cref{odewkb} to obtain the recursion relation
\begin{equation}\label{wkbrecur}
    x_0 = \pm i\om, \quad x_j = -\frac{1}{2x_0}\left( \dot{x}_{j-1} + \sum_{k = 1}^{j-1}x_k x_{j-k} \right), \quad \text{for } j = 1, 2, \ldots, N.
\end{equation}
