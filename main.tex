...
% This is because om(t) << int_0^t om(s) ds usually
In particular, scaling $\om(t)=\om_0 \Omega(t)$ as in \cref{introduction},
...
The above motivates our use of the following.
\begin{defn}[Condition number and achievable accuracy for an oscillatory region]
  Let $u$ be a solution to \cref{ode} on $(a,b)$ of the form
  ...
  by
  \be
  \kappa := \im\, [ z(b) - z(a) ], \label{conditionnodef}
  \ee
  and the resulting best achievable accuracy using finite precision by
  \be
  \varepsilon := \kappa \cdot \varepsilon_{\mathrm{mach}}.  \label{mineps}
  \ee
\end{defn}
...
we estimate $\kappa$ % (the total unsigned phase accumulated)
...
In the sections to follow we denote absolute error in a numerical solution by
...
32-core, 3.6 GHZ workstation with 264 GB of RAM.
% FA: *** Seriously, that much RAM?! I got this from cat /proc/meminfo, by looking at the MemTotal field.
