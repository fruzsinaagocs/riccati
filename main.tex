...
%and will therefore be referred to as the phase function method hereafter.
...
%\Fruzsi{Need clarification about this comment.} *** what do you want to say?

% US
Here we present an approach using an asymptotic expansion for a nonoscillatory Riccati phase function, meaning simply that the phase function is the logarithmic derivative $x(t) = (\log u(t))'$.
% AB: I cut since it's confusing, there is no om_0 scaling in our formulae.
% FA: I think it's more confusing this way... I want to tell the reader which
% symbol corresponds to the phase function, to avoid confusing it with Bremer's
% definition of the phase function! It's not clear this way, we never actually
% told them before this sentence what our Riccati phase function is. I changed
% it again (but not back to how it was).
...
\cite{tref}.  % AB added this out of respect
...
\begin{pro}\label{PRiter}  % ppppppppppppppppppppppppppppppppppppppppppppppp
  ...
  \ee
\end{pro}                  % pppppppppppppppppppppppppppppppppppppppppppppp
...
\begin{rmk}\label{r:fewer}  % rrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrr
...
\end{rmk}                    % rrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrr
% AB notice how I was explict about the eps-dependence, very easy to be.


\begin{figure*}[tb]  % ffffffffffffffffffffffffffffffffffffffffffffffffffff
    \flushleft
...
\end{figure*}    % ffffffffffffffffffffffffffffffffffffffffffffffffffffffffff
...
$u(t_i)$ and $u'(t_i)$, as needed for a 2nd-order ODE timestepper.
...
% *** AB fixed typo here
\ee
Since $\om$ and $\gamma$ are real, they are complex conjugates.
...
\end{bmatrix}.
\ee
Here we used that $u_{\pm}(t_i) = 1$ and $u'_{\pm}(t_i) = x_{\pm}(t_i)$ by construction of \eqref{upm}.
...
converge before the residual reaches a user-specified tolerance $\varepsilon$, we switch to a standard spectral collocation method on a Chebyshev grid.
...
= u(t_i), \qquad \left(D_n\mbf{u}\right)_0 = u'(t_i).
\ee
...
\end{bmatrix},
\ee
which is solved in the least-squares sense via QR with $\bigO(n^3)$ cost.
% AB *** please check - we need to say how.
$u(t)$ at interval endpoint can be read off as the last element of the
solution vector $\mbf{u}$, and its derivative as the last element of
$D_n\mbf{u}$.
...
but that we did not find that this made a significant speed difference
in practice.)
...
algorithm. The left side of Figure~\ref{balls-flowchart} shows
a summary flowchart.
...
use the high-frequency Riccati solver or the plain spectral method.
...
Recalling \eqref{x1R1}, the first correction term is $\om'/\om$, giving the
...
In practice to approximate $\om'(t_i)$ use Chebyshev differentiation
from the grid of the previous timestep.
...
The discretization in \cref{s:discr} is certainly no more accurate than
...
% AB *** note I fixed your formula, but you have to check it
...
% AB *** I don't understand this since they fall outside interval, plus why \pi is here??
%\ee
...
the defect correction of Section~\ref{phasefun}
...
\begin{thm}\label{TR}   % ttttttttttttttttttttttttttttttttttttttttttttttttt
...
\begin{rmk}\label{slight}
  ...
  In particular, by the inequalities \eqref{ommag}--\eqref{gammaupper} and
  definitions \eqref{eta1}--\eqref{eta3},
...
Using \eqref{eta1} for $\te_1$ and \eqref{eta3} for $\te_3$ we write this upper bound as
$$
\left|\sum_{\ell=0}^{j} \frac{R_\ell}{2\left( x_\ell + \g \right)} \right|
\leq
2\frac{\eta_3 + 2\eta_2\eta_4}{\eta_1 - \eta_4 - \frac{17\eta_3}{4\eta_1} - \frac{17\eta_2\eta_4}{2\eta_1}},
$$
then extend it by lower-bounding the denominator via \eqref{omder} and the crude bound ${\eta_4 \leq \eta_1/34}$ from \eqref{gammaupper}. This gives
$$
\left|\sum_{\ell=0}^{j} \frac{R_\ell}{2\left( x_\ell + \g \right)} \right| 
\leq
...
\begin{cor}[Superasymptotic approximation]\label{super}
  Suppose $\om$ and $\g$ satisfy the conditions \eqref{ommag}--\eqref{gammaupper}
...
a refinement of Remark~\ref{r:fewer}.
%*** compare to numerics.
