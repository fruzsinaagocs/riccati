...
course of the solution interval. 
The algorithm automatically switches between two methods for dealing with the
two types of behaviour and has $\bigO(1)$ runtime in terms of the frequency
of oscillations in the highly oscillatory regime. It is arbitrarily high
order accurate in both the oscillatory and the slowly changing regime.
When the solution changes slowly a spectral methods based on $n$ Chebyshev
nodes is used, where $n$ is changed adaptively to ensure the local error is
within a user specified tolerance. During a highly oscillatory phase, the
...
``phase function'') via an asymptotic series. The number of terms included
in the series is also adaptive. 
We describe in detail each method making up the algorithm and analyse their
error. We prove that the asymptotic series achieves temporary
...
relative to its magnitude. We demonstrate the efficiency and applications of
...
on $n$ Chebyshev nodes, where $n$ is chosen adaptively, resulting in an
$\bigO(h^{n+1})$ local error, where $h$ is the stepsize. This is in contrast to
...
numerically on an $(n+1)$-point Chebyshev grid (with $n \approx 16$). Ultimately, it is how well $\om$ and $\g$ can be represented by $n$th
...
$\Delta$ exceeds $\varepsilon_h$, using an exponent slightly larger than $1/n$ takes
...
% Future work
Within the scope of this work, we only allowed $\om^2 \geq 0$
Allow for negative $\om$, evanescent solutions

Boundary value problems by determining the switching points a priori

Error analysis: relating the residual to the error in the numerical solution

PDEs

Break degeneracy between $n$ and $h$ in Chebyshev steps -- Dan's paper

Fast quadrature of oscillatory functions?
...
% Figure for complex plane setup
...
%Introduction
%\begin{itemize}
%    \item{Summary of previous efforts: define asymptoticity, WKB theory, QLM, oscode. Non-asymptotic: Arnold, Bremer}
