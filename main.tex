...
\label{iter}
\end{align}
This is justified heuristically as an {\em approximate} Newton
iteration for \eqref{ricc} (noting that an exact Newton iteration has
been used numerically \cite{krivec2008,krivec2014}
and as an analysis tool \cite{heitman2015}).
Namely, perturbing by a function $\delta(t)$ we get
\begin{align}
    R[x_j + \delta] &= x_j' + \delta' + x_j^2 + 2 x_j \delta + \delta^2 
    + 2\g x_j + 2\g\delta + \om(t)^2 \nonumber \\
    &= R[x_j] + \delta' + 2x_j\delta + 2\g\delta + \bigO(\delta^2), \nonumber
\end{align}
then by linearizing we get that $\delta$ solves the linear 1st-order ODE
...
with unknown initial condition needed to achieve a nonoscillatory
solution $\delta(t)$.
...
writing $x_{j+1} = x_j + \delta$ gives our proposed \cref{iter}.
...
the residual reduction. We show the case $\g(t) \equiv 0$, for
simplicity, but the results hold for the general case.
The first few iterates and residuals are
\begin{align}
...
\end{align}
To interpret this, we assume that $\om'$, $\om''$, \etc, are
...
one higher derivative of $\om$ (and $\g$).
%Despite the rapid growth in complexity,
Algebraically, one can check that this
pattern continues
