...
\begin{description}
    ...
    \item[$\bm{t_{\mathrm{solve}}}$:]{Total runtime, in seconds, of a single
        ...
        \cite[Table~1]{bremer2018}. Note that this alone is not a good basis for
...
%        used for the sake of comparison on its own.}
    ...
    \item[$\bm{n_{\mathrm{LS}}}$:]{Total number of linear solve
...
this regime, but roughly 10 times faster. Towards $\lambda = 10^7$, only
...
% Also I changed 1e-6 to 1e-7 since I know that's typ for Bremer results.
...
$\mathcal{O}(10^{-7})$ \si{\s} per new target time.  
