...
\begin{thm}\label{t:R}
  ...
  the function $x_j$ has residual \eqref{R} bounded at the point $t$ by
  \be
  |R_j(t)| \;\le\; \eta_3 r^j~.
  \ee
\end{thm}
This shows temporary geometric convergence up to $k$ iterations,
...
%
\begin{proof}
  Define the concentric nested set of closed balls $B_j := B_{\rho(1-j/k)}(t)$, $j=0,1,\dots,k$, such that
  $B_0$ is the original ball in the statement of the theorem, while
  $B_k = \{t\}$ is the single point of interest.
  %The functional iteration \eqref{init}--\eqref{iter},
  We use induction in $j$.
%  and consider the iteration acting on the functions
  We take as the induction hypotheses that $x_j$ is analytic in $B_j$, with
  \bea
  \te_1 \;\le\; |x_j| &\le& \te_2  \quad \mbox{ in } B_j~,
  \label{hypx}
  \\
  |R_\ell| &\le& \eta_3 r^\ell \quad \mbox{ in } B_j~, \mbox{ for all } \ell = 0,1,\dots,j~.
  \label{hypR}
  \eea
  %
  For an analytic function $y$ we use the notation
  $\|y\|_j := \max_{z \in B_j}|y|$ for the $\infty$-norm on the $j$th ball.
  Cauchy's theorem for derivatives states that if $y$ is analytic
  and bounded in $B_j$, then for any $z$
  $$
  y'(z) = \frac{1}{2\pi} \int_{|\zeta-t| = (1-j/k)\rho} \frac{d\zeta}{(
  $$
  and thus for any $z\in B_{j+1}$
  
  \eqref{t:Riter} then implies
$$
  \|R_{j+1}\|_{j+1} = \frac{1}{2x_j}\biggl(
  \frac{x_j'}{x_j} R[x_j]
  - R[x_j]'
    \biggr) + \biggl(\frac{R[x_j]}{2x_j}\biggr)^2~.
$$
...
*** Connect $R$ to error in sat the ODE,
then from this sim to local discretization err
