...
Consider $n+1$ Chebyshev nodes over the standard interval $t \in [-1, 1]$, 
\be\label{chebnodes}
\tilde{t}_j = \cos\left( j\pi/n\right), \quad j = 0, 1, \ldots, n.
\ee
One may define an $n+1 \times n+1$ differentiation matrix $D_n$ which, given a vector of function
values $v_n$ defined on the Chebyshev nodes, returns $w_n$, the function's
derivative evaluated at the nodes,
\be\label{diffmatrix}
w_n = D_nv_n.
\ee
Using this differentiation matrix \cref{ode} may then be discretized over the interval $t \in [t_i, t_i+h]$ as
\begin{align}\label{discreteode}
    \tilde{D}_n u_n = \left(\left(\frac{2}{h}\right)^2D_n^2 + 2\frac{2}{h}\text{diag}(\g_n)D_n + \text{diag}(\om_n) \right)u_n = 0, 
\end{align}
where the subscript $n$ denotes a vector of function values at the $n+1$ Chebyshev nodes
\be\label{scaledt}
...
\be\label{discreteic}
u_n[0] = u(t_i), \quad D_nu_n[0] = u'(t_i).
\ee
The initial conditions may be encoded as two rows appended to $\tilde{D}_n$,
\be\label{discreteodeic}
\renewcommand*{\arraystretch}{1.25}
\begin{bmatrix}
    \tilde{D}_{n, 00} & \ldots & \tilde{D}_{n, 0n} \\
    \vdots & \ddots & \vdots \\
    \tilde{D}_{n, n0} & \ldots & \tilde{D}_{n, nn} \\
    \frac{2}{h}D_{n, 00} & \ldots & \frac{2}{h}D_{n, 0n} \\
    1 & \ldots & 0
\end{bmatrix}
u_n =  
\begin{bmatrix}
...
\end{bmatrix},
\ee
where the last two lower indices specify a given element of a matrix.
...
solution vector $u_n$, and its derivative as the last element of $2/hD_nu_n$.
% Solution of rectangular systems slower
% Overdetermined, can kill 2 rows to make system square
