...
in a ball,
by choosing a roughly optimal stopping $k$ one can achieve
exponential convergence with respect to the size of the frequency $\om$,
as follows.
*** insert Boyd and Berry reviews on superconvergence.

\begin{cor}[Superconvergence]\label{super}
  Suppose $\om$ satisfies the conditions \eqref{ommag}--\eqref{omder}
  of Theorem~\ref{t:R}
  about a point $t\in\R$,
  and let $\alpha := (1+\te_2/\te_1)/2\te_1\rho$ be the
  first term in the rate \eqref{r} arising from these conditions.
...
Since this optimal number of iterations grows as $k = \bigO(\om)$,
for large $\om$ it is impractical.
