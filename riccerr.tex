...
\end{align}
$D_n$ can then be used to compute antiderivatives (\eg $v_n$ given $w_n$), by
solving the matrix equation \cref{scaledDdef}. 

% Initial conditions

We ensure continuity of the solution and its derivative across timestep boundaries, 
...
their estimated error (as done in ... TODO), but in order to minimise
...
practice, since over $h_{\text{osc, slo}}$, $\om$, $\om'$, and $\g$ may change significantly. 
...
Starting from $h_{\text{osc}}$, we wish to determine 
