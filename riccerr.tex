...
\begin{rmk}
  ...
  the number of oscillation periods across the ball diameter divided by $\pi$.
  Then, for example, with 100 periods across the ball, $r$ satisfies \eqref{r}
  for $k\le 25$.
\end{rmk}
%
\begin{proof}
...
$$
x_{j+1}(t) = i\om(t) - \sum_{\ell=0}^{j} \frac{R_\ell(t)}{2 x_\ell(t)}~.
$$
Using the hypothesis, the second term is pointwise bounded in magnitude
in $B_{j+1}$ by
$$
...
In the proof, the choice of $4/5$ in \eqref{r} was merely a convenient one,
chosen so that the bounds $[\te_1,\te_2]$ on $|x_j|$
