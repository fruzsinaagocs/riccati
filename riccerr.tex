...
summarized by the following, which comes simply by
substituting $\delta_j := -R[x_j]/2x_j$ into the above.
...
Proposition~\ref{p:Riter} described the evolution of
the residual function under the proposed iteration.
We now abbreviate $R_j = R_j(t) := R[x_j](t)$.
Examining \eqref{Riter}, since $x_j'$ and $x_j$ are of order $\om$
then if $R_j$ and $R_j'$ are sufficiently small
the last term is negligible, so that $R_j$ is reduced at each point $t$
by roughly $x_j(t)$, which is $\bigO(\om)$.
However, because there is one extra derivative per iteration,
this apparent geometric convergence is misleading---the series
is merely asymptotic in the large parameter $\om$.
Note that the iteration for $R_j$ is local in $t$.
We now make this argument rigorous, when $\om$ is analytic
and sufficiently large throughout some neighborhood of $t$.

\begin{thm}\label{t:R}
  For now fix $\rho=1$ ***.
  Fix $t\in\R$ and let the frequency function $\om$ be analytic
  in the closed ball $B_\rho(t) := \{z\in\C : |z-t| \le \rho\}$ and
  \bea
  \eta_1 \;\le\; |\om(z)| &\le& \eta_2~, \qquad \forall z\in B_\rho(t)
  \\
  |\om'(z)| &\le & \eta_3~, \qquad \forall z\in B_\rho(t)
  \eea
  Let $k\in\N$ be sufficiently small such that
  \be
  r := \frac{1}{2\te_1}
  \biggl[ \biggl(1 + \frac{\te_2}{\te_1}\biggr) k + \frac{\eta_3}{2\te_1}\biggr]
  \; \le \; \frac{4}{5}
  \ee
  where $\te_1:=\eta_1 - 3\eta_3/\eta_1$ and $\te_2:=\eta_2 + 3\eta_3/\eta_2$.
  Then after $k$ iterations of \eqref{init}--\eqref{iter}, the residual
  is bounded by
  \be
  |R_k(t)| \;\le\; \eta_3 r^k~.
  \ee
\end{thm}
This guarantees temporary geometric convergence at a rate that deteriorates
with $k$, the stopping iteration number.
For any $k$ to exist satisfying the condition on $r$, $\om$ must have
a sufficiently large lower bound $\eta_1$.
Note that $[\te_1,\te_2]$ is a slight expansion of the interval containing
the range of $|\om|$ in the ball.
%
\begin{proof}
  Define the concentric nested set of balls $B_j := B_{\rho(1-j/k)}(t)$, such that
  $B_0$ is the original ball in the statement of the theorem, while
  $B_k = \{t\}$ is the single point of interest.
  Consider the functional iteration for $x_j$
  We use induction in $j$, and consider the iteration acting on the
  functions
  We take as the induction hypotheses that
  \bea
  \te_1 \;\le\; |x_j| &\le& \te_2  \mbox{ in } B_j~,
  \label{hypx}
  \\
  |R_\ell| &\le& \eta_3 r^\ell \mbox{ for all } \ell = 0,1,\dots,j~.
  \label{hypR}
  \eea
  %
  ***
  %
  Finally, the base case $j=0$ satisfies \eqref{hypx}--\eqref{hypR}
